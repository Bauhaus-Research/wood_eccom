<script>
	...
	import {products, cart, totalPrice, totalItems} from "$lib/stores/Cart"
...
>
	<div class="flex flex-row justify-center items-center text-center ml-2 md:ml-14">
		...
		<div class="mx-1 font-black lg:hidden">|</div>
		<a href="/" class="text-sm w-fit md:text-xl font-black">Wood You Furniture</a>
	</div>
	<ul class="flex flex-row justify-end items-center text-center mr-2 md:mr-14">
		<div class="lg:block hidden w-0 lg:w-fit">
			{#each navItem as items}
				<li class="fade-underline md:mx-3 mx-0"><a href={items.link}>{items.title}</a></li>
			{/each}
		</div>
		<div class="{$totalItems==0?'hidden': ''} py-1 px-3 text-white text-opacity-70 translate-x-4 translate-y-4 rounded-full bg-black bg-opacity-30">
			{$totalItems}
			</div>
			<a href="/cart" class="h-8 mr-4 ">
				<svg
					class="{scrollY > 10 || showTapMenu == true
						? 'text-green-900'
						: 'text-white'} {$totalItems===1? 'animate-pulse text-opacity-80':$totalItems===2? 'animate-pulse duration-[10ms] text-opacity-80':$totalItems===3? 'animate-pulse  text-opacity-90':$totalItems>=4? 'animate-bounce text-opacity-100 text-green-900':''} h-full w-full hover:fill-current group-hover:text-green-900 transition-all"
		...
		<a href="/favourites" class="h-8 mr-4">
			<svg
				class="{scrollY > 10 || showTapMenu == true
					? 'text-green-900'
					: 'text-white text-opacity-40'} h-full w-full hover:fill-current group-hover:text-green-900 transition-all"
				width="800px"
				height="800px"
				viewBox="0 0 24 24"
				fill="none"
...
<style>
	...
	.fade-underline::before {
		...
		background-color: rgb(20, 83, 45); /* Change to your desired color */
